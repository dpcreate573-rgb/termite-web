...
export const QuoteFormA = memo(function QuoteFormA({ onUpdate, subject = "" }: { onUpdate: (data: { total: number; items?: LineItem[] }) => void, subject?: string }) {
  const [items, setItems] = useState<TermiteItem[]>(() => [
    { id: Date.now(), name: "", price: 0, qty: 1, unit: "式" }
  ])
  ...
  // 初期化時にシロアリ駆除カテゴリの品目マスターを取得
  useEffect(() => {
    getItemsByCategory("シロアリ駆除").then(items => {
  ...
  const updateItem = (id: number, field: keyof TermiteItem, value: string | number) => {
  ...
  const handleRegisterItem = async (item: TermiteItem) => {
    ...
    try {
      ...
      // Optional: Show a brief success indicator (e.g. alert or toast, here just alert for simplicity or nothing if smooth)
  ...
  const total = items.reduce((sum, item) => sum + Math.floor(item.price * item.qty), 0)
  ...
  const buildItems = () => {
    return items
      ...
      .map(item => ({
        ...
        unit: item.unit || "式",
        price: item.price,
        amount: Math.floor(item.price * item.qty)
      }))
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-2">
        <h3 className="font-semibold text-blue-900">シロアリ駆除 施工内容</h3>
      ...
      <div className="space-y-3">
        {/* ヘッダー */}
        <div className="hidden md:flex gap-2 px-2 text-xs font-semibold text-gray-500">
          <div className="flex-[2]">品目名 / 作業名</div>
          <div className="w-24 text-right">単価</div>
          <div className="w-20 text-right">数量</div>
          <div className="w-20">単位</div>
          <div className="w-24 text-right">金額</div>
          <div className="w-10"></div>
        </div>

        {items.map((item) => (
          <div key={item.id} className={`flex flex-col md:flex-row gap-2 items-start md:items-center bg-gray-50/50 p-3 md:p-2 rounded-md border border-gray-100 relative ${activeInputId === item.id ? 'z-50' : 'z-10'}`}>
            ...
            <div className="flex w-full md:w-auto gap-2">
              ...
              <div className="flex-1 md:w-24">
                ...
                <Input
                  type="text"
                  value={item.price === 0 && item.name === '' ? '' : item.price.toLocaleString()}
                  onChange={(e) => {
                    const rawValue = e.target.value.replace(/,/g, '')
                    const numValue = parseInt(rawValue, 10)
                    updateItem(item.id, 'price', isNaN(numValue) ? 0 : numValue)
                  }}
            ...
            <div className="flex items-center justify-between w-full md:w-auto mt-2 md:mt-0 pt-2 md:pt-0 border-t md:border-t-0 border-gray-100">
              ...
              <div className="w-24 text-right font-medium text-gray-800">
                ...
                ¥{Math.floor(item.price * item.qty).toLocaleString()}
              </div>
      ...
      <div className="pt-4 border-t flex justify-end">
        <div className="text-right">
          <p className="text-sm text-gray-500 mb-1">シロアリ駆除 計</p>
