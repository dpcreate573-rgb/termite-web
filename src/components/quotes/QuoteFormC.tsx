...
export const QuoteFormC = memo(function QuoteFormC({ onUpdate, subject = "" }: { onUpdate: (data: { total: number; items?: LineItem[] }) => void, subject?: string }) {
  const [items, setItems] = useState<OtherItem[]>(() => [
    { id: Date.now(), name: "", price: 0, qty: 1, unit: "個" }
  ])
  ...
  // 初期化時にその他カテゴリの品目マスターを取得
  useEffect(() => {
    getItemsByCategory("商品・物販・その他業務").then(data => {
  ...
  const addItem = () => {
    setItems([...items, { id: Date.now(), name: "", price: 0, qty: 1, unit: "個" }])
  }
  ...
  const updateItem = (id: number, field: keyof OtherItem, value: string | number) => {
  ...
  const handleRegisterItem = async (item: OtherItem) => {
    if (!item.name || item.name.trim() === '') return;

    setIsSavingItem(prev => ({ ...prev, [item.id]: true }));
    try {
      await createItem({
        category: "商品・物販・その他業務",
        name: item.name,
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center mb-2">
        <h3 className="font-semibold text-orange-900">その他 明細</h3>
      ...
      <div className="pt-4 border-t flex justify-end">
        <div className="text-right">
          <p className="text-sm text-gray-500 mb-1">その他 計</p>
